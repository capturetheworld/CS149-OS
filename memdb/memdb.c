...
int main(int argc, char **argv) {
 ...
 struct entry_s create(char inputString[]){

    struct entry_s toAdd = {.len = strlen(inputString) +1, .magic = ENTRY_MAGIC_DATA }; //from .h
...
moffset_t add(int fd, char inputString[]) {
    ...
    struct entry_s toAdd = create(inputString); //create a node
    struct entry_s* pointer_toAdd = malloc(sizeof(toAdd) + toAdd.len); //create a pointer



    memcpy(pointer_toAdd, &toAdd, sizeof(toAdd));
    memcpy(sizeof(toAdd) + (char*) pointer_toAdd, inputString, toAdd.len);
    ...
    if (!fhdr->data_start) {
        ...
        memcpy(ptr + offset, pointer_toAdd, sizeof(toAdd) + pointer_toAdd->len);
        fhdr->data_start = fhdr->free_start;
        fhdr->free_start += sizeof(toAdd) + pointer_toAdd->len;
